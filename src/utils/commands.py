...
async def ask_taskdate(update, context):
    ...
    if not name:
        return conversation_cancel(update, context)
    ...
    return VALIDATE_ADD # Redireciona para ask_task_date no conversation_handler
...
async def ask_test_date(update, context):
    ...
    if context.user_data["test_operation"] == "remove":
        return VALIDATE_TEST_REMOVE
    return VALIDATE_TEST_ADD
...
async def validate_addtest(update, context):
    ...
    try:
        datetime.strptime(date, "%d/%m/%Y")
        jq.set_test_timer(update, context)
...
async def validate_removetest(update, context):
    ...
    job_removed = jq.remove_jobs(context.user_data["test_name"], context)
...
async def start_pomodoro(update, context):
    ...
    try:
        if context.user_data["pomodoro"]:   
            await update.message.reply_text("Pomodoro ja está em execução")
            return ConversationHandler.END
    except KeyError:
        pass
...
async def ask_pomodoro_time(update, context):
    ...
    await update.message.reply_text("Quantos rounds?")

    return ASK_POM_ROUNDS
...
async def ask_pom_round(update, context):
    ...
    await update.message.reply_text("Quantos minutos de descanso?")
...
async def ask_break_time(update, context):
    ...
    jq.set_pomodoro_job(update, context)
    context.user_data.clear()
...
async def cancel_pomodoro(update, context):
    ...
    removed = jq.remove_jobs("Pomodoro", context)
